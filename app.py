...
if uploaded_file:
    try:
        ...
        # Ensure sheet is loaded before performing checks
        if sheet:
            ...
            id_values = [sheet.cell(row=row, column=1).value for row in range(2, 32)]
            ...
            # If the sheet is not found, mark remaining checks as "No"
            checklist_data["Completed"].extend(["No"] * (len(checklist_data["Grading Criteria"]) - 1))
